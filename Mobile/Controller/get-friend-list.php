...
while ($allUser = mysqli_fetch_assoc($Query1)) {
    ...
    if ($allUser['unique_id'] != $uniqueId) {
        ...
        if ($allUser['status'] === "Active now") {
            $statusIcon = '<i class="fa-solid fa-circle active"></i>';
        }
        ...
        <li class="flex items-center cursor-pointer">
            <a href="./chat-room.php?chooseFri=' . $allUser['unique_id'] . '" class="flex items-center w-full">
                <div class="con">
                    <img src="' . $profileImage . '" class="rounded-full border-color shrink-0 pro" />
                    ' . $statusIcon . '
                </div>
                <div class="ml-3">
                    <h1 class="whitespace-nowrap">' . $allUser['name'] . '</h1>
                    <p class="text-xs whitespace-nowrap text-muted">'.$howMess.' : '.$lastMessage["message"].'</p>
                </div>
            </a>
